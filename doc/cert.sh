...
if [ ! -f $1-req.pem ] ; then
  openssl req -new -nodes -out $1-req.pem -keyout ./private/$1-key.pem -days 3650 \
    -config ./openssl.cnf
fi

if [ ! -f $1-cert.pem ] ; then
  openssl ca -out $1-cert.pem -days 3650 -config ./openssl.cnf -infiles $1-req.pem
fi

if [ ! -f ../$1.p12 ] ; then
  openssl pkcs12 -export -in $1-cert.pem -inkey ./private/$1-key.pem \
    -certfile ca_cert.pem -out ../$1.p12
fi
