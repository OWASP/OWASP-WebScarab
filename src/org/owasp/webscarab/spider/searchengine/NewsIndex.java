...
{
	static MatchBox patCount = new RegExp( "<center>Headlines\\s+\\d+\\s+to\\s+\\d+\\s+of\\s+(\\d+)</center>" );
	static MatchBox patNoHits = new RegExp( "No articles were found matching your search criteria" );
	static MatchBox patResult = new TagExp( "<dd>(?{link}(?{title}<a>.*?</a>))" // title and link
